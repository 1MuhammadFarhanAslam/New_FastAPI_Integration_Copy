...
class MusicGenerationService(AIModelService):
    ...
    def get_filtered_axons_from_combinations(self):
        ...
        if self.combinations:
            ...
            bt.logging.info(f"Current Combination for TTM: [31,32]")
            filtered_axons = [self.metagraph.axons[i] for i in [31,32]]
        else:
            self.get_filtered_axons()
            current_combination = self.combinations.pop(0)
    ...
    def get_filtered_axons(self):
        ...
        queryable_uids = queryable_uids * [self.metagraph.neurons[uid].axon_info.ip != '0.0.0.0' for uid in uids]
        queryable_uid = queryable_uids * [
